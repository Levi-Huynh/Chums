...
namespace ChurchLib{
	...
	public partial class ServiceTime
	{
		...
		public ServiceTime(DataRow row)
		{
		...
		#region Methods
		public static ServiceTime Load(string sql, CommandType commandType = CommandType.Text, MySqlParameter[] parameters = null)
		{
			ServiceTimes serviceTimes = ServiceTimes.Load(sql, commandType, parameters);
			return (serviceTimes.Count == 0) ? null : serviceTimes[0];
		}

		public static ServiceTime Load(int id, int churchId)
		{
			return Load("SELECT * FROM ServiceTimes WHERE Id=@Id AND ChurchId=@ChurchId", CommandType.Text, new MySqlParameter[] { new MySqlParameter("@Id", id), new MySqlParameter("@ChurchId", churchId) });
		}
		...
		{
			string sql = "INSERT INTO ServiceTimes (ChurchId, ServiceId, Name, Removed) VALUES (@ChurchId, @ServiceId, @Name, @Removed); SELECT LAST_INSERT_ID();";
		...
		{
			string sql = "UPDATE ServiceTimes SET ChurchId=@ChurchId, ServiceId=@ServiceId, Name=@Name, Removed=@Removed WHERE Id=@Id AND ChurchId=@ChurchId; SELECT @Id;";
			MySqlCommand cmd = new MySqlCommand(sql, conn) {CommandType = CommandType.Text};
			cmd.Parameters.AddWithValue("@Id", (_isIdNull) ? System.DBNull.Value : (object)_id);
			cmd.Parameters.AddWithValue("@ChurchId", (_isChurchIdNull) ? System.DBNull.Value : (object)_churchId);
			cmd.Parameters.AddWithValue("@ServiceId", (_isServiceIdNull) ? System.DBNull.Value : (object)_serviceId);
		...
		{
			DbHelper.ExecuteNonQuery("DELETE FROM ServiceTimes WHERE Id=@Id AND ChurchId=@ChurchId", CommandType.Text, new MySqlParameter[] { new MySqlParameter("@Id", id), new MySqlParameter("@ChurchId", churchId)  });
		}

		public object GetPropertyValue(string propertyName)
		{
			return typeof(ServiceTime).GetProperty(propertyName, BindingFlags.IgnoreCase | BindingFlags.Public | BindingFlags.Instance).GetValue(this, null);
		}
		#endregion
	}
}
